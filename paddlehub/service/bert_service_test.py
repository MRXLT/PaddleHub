...
class BertService():
    ...
    def infer(self, request_msg):
        if self.load_balance == 'round_robin':
            ...
            try:
                ...
                connect_cost = connect_end - connect_start
                cur_con.request('POST', "/BertService/inference", request_msg,
                                {"Content-Type": "application/json"})
                request_cost = time.time() - connect_end
                response = cur_con.getresponse()
                ...
                response_msg = ujson.loads(response_msg)
                net_cost = time.time() - connect_end
                load_cost = time.time() - load_start

                return response_msg, connect_cost, net_cost, load_cost, read_cost, request_cost
            except BaseException as err:
    ...
    def encode(self, text):
        ...
        for run_step, batch in enumerate(data_generator(), start=1):
            ...
            json_cost = time.time() - json_start
            if self.show_ids:
                logger.info(request_msg)
            request_start = time.time()
            response_msg, connect_cost, net_cost, load_cost, read_cost, request_cost = self.infer(
                request_msg)
            retry = 0
            while type(response_msg) == str and response_msg == 'retry':
                if retry < self.retry:
                    ...
                    response_msg, connect_cost, net_costi, load_cost, read_cost, request_cost = self.infer(
                        request_msg)
                else:
        ...
        if self.profile:
            return [
                ...
                response_msg['infer_time'], copy_time, connect_cost, net_cost,
                json_cost, load_cost, read_cost, request_cost
            ]
        else:
            return result
...
def test():
    ...
    print(text[0], result[0][:10])
    print(text[1], result[1][:10])
    bc.close()
